...
@Autonomous(name = "Front 4 Cycle", group = "autonomous")
public class front4Cycle extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        TrajectoryActionBuilder toIntakeThree = drive.actionBuilder(poses.shootFrontPose())
                .setTangent(poses.shootFrontTanOut())
                .splineToSplineHeading(poses.startIntakeThreePose(), -poses.startIntakeThreeTan()) // move to prepare for intake again
                .strafeToConstantHeading(poses.endIntakeThree(), new TranslationalVelConstraint(17)); // intake again again!

        TrajectoryActionBuilder toShootFour = drive.actionBuilder(poses.endIntakeThreePose())
                .setTangent(poses.endIntakeThreeTan())
                .splineToSplineHeading(poses.shootFrontParkPose(), poses.shootFrontTanIn(), new TranslationalVelConstraint(27)); // go to shoot again again again! (outside the line)
                            ...
                            new SequentialAction(
                                    ...
                                    new ParallelAction( // scoop once and start spinning up on the way to shoot
                                            new SequentialAction(
                                                    ...
                                                    state.setBotAction(botState.shoot, false, false, false, shootRPM, shootAngle)), // start flywheel while bot is moving
                                            toShootFour.build()),
                                    state.setBotAction(botState.shoot, false, true, false, shootRPM, shootAngle), // shoot three balls (already out of the way)
