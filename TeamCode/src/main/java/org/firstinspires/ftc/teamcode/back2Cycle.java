...
@Autonomous(name = "Back 2 Cycle", group = "autonomous")
public class back2Cycle extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        TrajectoryActionBuilder toIntakeRow = drive.actionBuilder(poses.shootBackPose())
                .setTangent(poses.shootBackTanOut())
                .splineToSplineHeading(poses.startIntakeRowPose(), -poses.startIntakeRowTan()) // move to prepare for intake
                .strafeToConstantHeading(poses.endIntakeRow(), new TranslationalVelConstraint(10)); // intake!

        TrajectoryActionBuilder toShootTwo = drive.actionBuilder(poses.endIntakeRowPose())
                .setTangent(poses.endIntakeRowTan())
                .splineToSplineHeading(poses.shootBackPose(), poses.shootBackTanIn(), new TranslationalVelConstraint(12)); // go to shoot again... idk for some reason this needs to be slower to get the angle right

        TrajectoryActionBuilder toEndMove = drive.actionBuilder(poses.shootBackPose())
                .splineToSplineHeading(poses.endBackPose(), poses.endBackTan()); // get off line to get move points
                    ...
                    new ParallelAction(
                            new SequentialAction(
                                    ...
                                    new ParallelAction( // scoop once and start spinning up on the way to shoot
                                            new SequentialAction(
                                                    state.setBotAction(botState.intake, false, false, false, 0, 0), // scoop once
                                                    new SleepAction(.75), // wait for a stuck ball to come up
                                                    state.setBotAction(botState.shoot, false, false, false, shootRPM, shootAngle)), // start flywheel while bot is moving
                                            toShootTwo.build()),
                                    state.setBotAction(botState.shoot, true, false,false, shootRPM, shootAngle), // shoot a ball
                                    state.setBotAction(botState.shoot, true, false,false, shootRPM, shootAngle), // shoot a ball
